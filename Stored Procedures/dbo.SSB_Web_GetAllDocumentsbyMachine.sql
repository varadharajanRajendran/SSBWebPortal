...
/* Find Machine Model */
 ...
 /* Get List of Documents */
  ...
  FROM [SSBWebPortal].[dbo].[SSB_DocMgmt] DM
  INNER JOIN [SSBWebPortal].[dbo].[SSB_DocProcessGroup] PG On DM.DocProcessGroup=PG.pk
  INNER JOIN [SSBWebPortal].[dbo].[SSB_DocMachineMap] DMM on DMM.DocumentPK =DM.pk
  INNER JOIN [SSB].[dbo].[SSB_UnitGroup] UG ON DMM.MacinePK=UG.pk
  WHERE DM.[Status]=2 AND
        UG.[Description]=@Machinemodel
  ORDER BY PG.[Sequence] ASC
